...
```{r, echo=T, message=F, warning=F, eval=F}
fruits <- "apple_count <- str_count(fruits, "Apple")
apple_count # the final result should output 3
```
...
apple_count <- str_count(fruits, "Apple")
apple_count
```
...
### Solution
```{r echo=T, message=F, warning=F}
fruit_list <- c("Apple", "Banana", "Cherry", "Date", "Apple")
apple_present <- str_detect(fruit_list, "Apple")
apple_present
```
...
```{r, eval=FALSE, warning=FALSE, out.width="70%", fig.align="center"}
nyc_regents_scores[, total_freq := rowSums(.SD, na.rm = TRUE), .SDcols = patterns("subject")]
```
...
```{r, eval=FALSE, warning=FALSE, out.width="70%", fig.align="center"}
nyc_regents_scores |> 
ggplot(aes(x = score, y = total_freq)) +
  geom_bar(stat = "identity", fill = "brown") +
labs(x = "Score", y = "Score Frequency", title = "NYC Regents Scores Frequency") + 
  theme_minimal() +
